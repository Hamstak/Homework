...
int * readNumbers( int * total )
{
	...
	*total = 0;
	...
	int number = 0;
	printf("Please input positive integer numbers ( or zero ),\ninput (-1) to finish input: \n");	
	scanf("%d",&number);	
	do{
		if (*total == size && number >= 0){
			size = size * 2;			
			iptr = realloc(iptr,size * sizeof(int));
		}
		if(number >= 0){
			*(iptr + *total) = number;
			(*total)++;
...
int computeMedian(int *p, int n)
{
	...
	int temp;
	int indexSmallest;
	for (posFill = 0; posFill < n; posFill++){
		int current;
      indexSmallest = posFill;
		for (current = posFill + 1; current < n; current ++){			
