...
class AutoCompleteStudent extends JFrame implements KeyListener {
	...
	String popular[] = {"apple", "apply", "boy", "bike", "book", "brook", "brown",
			"car", "counter"};
	HashTable dictionary = (new HashTable()).insertFromFile("dictionary.txt");
	...
	public void keyPressed(KeyEvent e) {
		...
		if ( index < 0 ) {	
			...
			if ( Character.isAlphabetic(ch) && inWord ) {
                ...
                String[] strings = dictionary.get(partialWord).take(9).toArray();
                if(strings.length != 0){
                    input.setText(arrtoString(strings));
                }else{
			...
			output.setText(dictionary.get(partialWord).take(1).toArray()[0]);
			inWord = false;
			partialWord = "";
		}//end of outer else
