...
public class MyLinkedList implements Iterable<Object> {
    public static void main(String[] args) throws IOException {
        System.out.println("Begin unit tests for MyLinkedList.java");
        MyLinkedList test = (new WordProcessor()).extractAll("files/testfile1");
        System.out.println(test);
        System.out.println(test.containWord("testing", 1) + " Should return false");
        System.out.println(test.containWord("fscanf", 2) + " Should return true");
        System.out.println(test.containWord("fscanf", 10) + " Should return true");
        System.out.println(test);
        test.sortListOcc();
        System.out.println(test);
        System.out.println("List shoud be sorted by occurance, and then alphabetically");
    }
		...
		public void addOrdered(Comparator comp, WordItem dataToAdd){
            Node cur, prev;
            for (cur = head.next, prev = head; cur != null && (comp.compare((WordItem)cur.data, dataToAdd) < 0);
                    cur = cur.next){
                prev = cur;
            }
            prev.next = new Node(dataToAdd, cur);
        ...
        public void sort(Comparator comp){
            ...
            for (curr = head.next; curr != null; curr = curr.next){
                newList.addOrdered(comp, (WordItem)curr.data);
            }
            this.head.next = newList.head.next;
        }
	    public void sortListOcc(){
            sort(new OccuranceComparator());
        }
		...
		public boolean containWord(String word, int atLine) {
            ...
            Node curr;
            for (curr = head.next; curr != null; curr = curr.next){
                if (((WordItem)curr.data).getWord().equalsIgnoreCase(word)){
                    ((WordItem)curr.data).increaseCount();
                    ((WordItem)curr.data).addAtLines(atLine);
                    return true;
                }
            }
            return false;
